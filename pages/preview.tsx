...
const Background = styled.div`
  ...
  position: relative;
  /* animation: move 4s ease-in alternate infinite; */
`;
...
const MessageAlignment = styled.li`
  max-width: 100%;
  display: flex;
...
function Message({ message }: { message: Message }) {
  const inner =
    message.user === "2" ? (
      ...
      <ReceivedMessage avatar={message.avatar} content={message.content} />
    );
  return (
    <MessageAlignment self={message.user === "2"}>
      <MessageContent>{inner}</MessageContent>
    </MessageAlignment>
  );
}
